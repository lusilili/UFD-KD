...
class VisionTransformerN(BaseBackbone):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(VisionTransformerN, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(VisionTransformerN, self).init_weights()
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in index:
                outs.append(self._format_mid_feat(x, patch_resolution))
    ...
    def _format_mid_feat(self, x, hw):
        patch_token = x[:, self.num_extra_tokens:]
        B = x.size(0)
        # (B, N, C) -> (B, H, W, C) -> (B, C, H, W)
        return patch_token.reshape(B, *hw, -1).permute(0, 3, 1, 2)
    ...
    def forward_specific_stage(self, x, stage, patch_resolution=(14,14)):
        N, L, C = x.shape
        if stage == 4:
        ...
        # x = x.permute(0,2,1).contiguous().view(N, C, patch_resolution[0], patch_resolution[1])
