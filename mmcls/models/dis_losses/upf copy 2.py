...
class ParameterTuningNetwork(nn.Module):
    def __init__(self):
        ...
        # 设计一个简单的全连接网络
        self.fc1 = nn.Linear(4, 128)  # 输入四个参数
        self.fc2 = nn.Linear(128, 128) # 隐藏层
        self.fc3 = nn.Linear(128, 4)  # 输出四个调优后的参数
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(self.fc3(x))  # 最后一层 Sigmoid 激活，确保输出在 [0, 1] 范围内
        return x
...
class UnifiedPathDynamicWeightFreqLoss(nn.Module):
    ...
    def forward(self,
        ...
        """
        # 输入当前的超参数给优化网络，得到调优后的参数
        optimized_params = self.parameter_tuning_network(self.alpha_jfd_tensor)

        # 获取调优后的参数
        self.alpha_dc, self.alpha_ac, self.alpha_dc_1d, self.alpha_ac_1d = optimized_params
