...
class JFDLossv2Norm(nn.Module):
    ...
    def __init__(self,
        ...
        super(JFDLossv2Norm, self).__init__()
        self.alpha_jfd = alpha_jfd
        self.projector_1 = AttentionProjector(student_dims, teacher_dims, hw_shapes, pos_dims, window_shapes=window_shapes, self_query=self_query)
        self.projector_2 = AttentionProjector(student_dims, teacher_dims, hw_shapes, pos_dims, window_shapes=window_shapes, self_query=self_query)
    ...
    def project_feat(self, preds_S, query=None):
        if query is None:
            out_1 = self.projector_1(preds_S, query)
            out_2 = self.projector_2(preds_S, query)
        else:
            out_1 = self.projector_1(preds_S, query[0])
            out_2 = self.projector_2(preds_S, query[1])
